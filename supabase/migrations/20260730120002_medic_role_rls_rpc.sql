-- =============================================================================
-- WHFC Inventory — wire the 'medic' role into RLS and RPCs
--
-- Medic is a limited clinical role: can view active items/locations/on-hand
-- quantities (already covered by existing is_active_user()-gated select
-- policies), use stock (consume_inventory), execute procedure kits
-- (dispense_kit), and view their own recent transactions. Medic cannot
-- receive inventory, transfer, adjust, run physical counts, dispose/adjust
-- lots, manage master data, or access administration — those RPCs and RLS
-- policies are left untouched (medic is simply not in their allow-lists).
-- =============================================================================

-- ---------------------------------------------------------------------------
-- inventory_transactions — medic sees only its own ledger rows, same
-- narrowing already applied to 'staff' (see inventory_transactions_select_staff_own).
-- ---------------------------------------------------------------------------
create policy inventory_transactions_select_medic_own
...
-- ---------------------------------------------------------------------------
-- consume_inventory — add medic ("use stock")
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- dispense_kit — add medic ("execute procedure kits").
-- Body copied verbatim from 20260708120003_dispense_history_fields.sql, only
-- the require_roles() array changes.
-- ---------------------------------------------------------------------------
