-- =============================================================================
-- WHFC Inventory — procedure kit gaps: display order, instructions,
-- admin-configured dosage presets, and medic own-row dispense visibility.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- procedure_kits.instructions — optional internal notes shown to staff before
-- dispensing (not a patient/clinical record field).
-- ---------------------------------------------------------------------------
alter table public.procedure_kits
  add column instructions text;

alter table public.procedure_kits
  add constraint procedure_kits_instructions_length
    check (instructions is null or char_length(instructions) <= 2000);

-- ---------------------------------------------------------------------------
-- procedure_kit_components.display_order — admin-controlled ordering,
...
alter table public.procedure_kit_components
  add constraint procedure_kit_components_dosage_presets_valid
    check (
...
alter table public.procedure_kit_components
  add constraint procedure_kit_components_dosage_presets_requires_variable
    check (dosage_presets is null or is_variable_quantity = true);

-- ---------------------------------------------------------------------------
-- dispense_kit — order components by display_order, then created_at.
-- Body otherwise identical to 20260730120002_medic_role_rls_rpc.sql.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- dispense_events / dispense_event_lines — medic own-row visibility, same
-- narrowing already applied to 'staff'.
-- ---------------------------------------------------------------------------
create policy dispense_events_select_medic_own
...
create policy dispense_event_lines_select_medic_own
