...
import type { ProcedureUsage } from "@/lib/dispense/analytics";
...
type Row = ProcedureUsage & { valueLabel: string };

function truncate(value: string, max = 22): string {
  return value.length > max ? `${value.slice(0, max - 1)}…` : value;
}

export function ProcedureUsageChart({ items }: { items: ProcedureUsage[] }) {
  const rows: Row[] = items.map((item) => ({
    ...item,
    valueLabel: formatQuantity(item.dispenseCount),
  }));
